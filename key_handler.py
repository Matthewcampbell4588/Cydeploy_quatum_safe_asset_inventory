...
#creates a signiture with the message and dilithium private key
def message_signing(dilithium_private_key,msg):
    ...
    print(f"[DEBUG] Dilithium key type: {type(dilithium_private_key)}")
    signiture  = dilithium_private_key.sign(msg)
    return signiture

#checks msg and signiture with dilithium public key
def message_verfication(msg,sig_msg,dilithium_pub):
    with oqs.Signature('Dilithium2') as verify:
        is_valid = verify.verify(msg,sig_msg,dilithium_pub)
...
def key_recv(client_socket):
        ...
        data = send_recv.recv(client_socket)
